...
The original presentation is from the Mastering Shiny repo [tidy evaluation repo](https://github.com/hadley/mastering-shiny/blob/master/action-tidy.Rmd), or the fully rendered page is available at [tidy evaluation](https://mastering-shiny.org/action-tidy.html).
***I have made modifications where I found them useful.***

When programming and particularly within functions and when referencing variables that you may want to change in the future, you have likely encountered the challenge of programming with tidy evaluation (also known as NSE [Non-standard evaluation]).
Early on in `rlang` tidy evaluation was primarily known as NSE so if you get stuck when googling tidy evaltion, I would recommend also looking for NSE.
...
***The original chapter used dplyr and ggplot2 in a Shiny app. This will be a slight variation on that. Particularly I have removed the references to shinytest that would take screen shots of the app and allow you to modify them, as this need an additional file to be referenced, and it expanded outside of a single markdown.***
...
***Upfront - your best option other than tidy evaluation to unquote quoted variables is eval(parse(text = <YOUR QUOTED VARIABLE>)), there have been times where I have had this work well, but generally I have found it to be overly buggy. That being said if you are required to use base packages it may be your best option.***
...
-   An **env-variable** (environment variable) is a "programming" variables that you create with `<-`.
    `input$var` is a env-variable.

-   A **data-variable** (data frame variables) is "statistical" variable that lives inside a data frame.
    `carat` is a data-variable.

***I tend to think of these are the unresolved (env-variable) and resolved (data variable), in that when it isn't working, it makes the most sense to me to think, well that just means that the variable wasn't correctly resolved so it is pulling something else.***
...
[^action-tidy-2]: In Shiny apps, the most common form of indirection is having the name of data-variable stored in a reactive value.
    There's another form of indirection that happens when you're writing functions which is solved using `{{ x }}`, called embracing.
...
```{r tidied-up, fig.cap = "Our app works now that we've been explicit about `.data` and `.env` and `[[` vs `$`.", echo = FALSE, message = FALSE, out.width = NULL}
num_vars <- c("carat", "depth", "table", "price", "x", "y", "z")
shinyApp(
  ui = fluidPage(
    selectInput("var", "Variable", choices = num_vars),
...
The above figure shows that we've been successful --- we only see diamonds with values of `carat` greater than 1.
...
```{r Sepal.Width, message = FALSE, out.width = NULL}
shinyApp(
  ui = fluidPage(
    selectInput("x", "X variable", choices = names(iris)),
    selectInput("y", "Y variable", choices = names(iris), selected = "Sepal.Width"),
    plotOutput("plot")
  ),
  server = function(input, output, session) {
...
```
```{r species, message = FALSE, out.width = NULL}
shinyApp(
  ui = fluidPage(
    selectInput("x", "X variable", choices = names(iris)),
    selectInput("y", "Y variable", choices = names(iris), selected = "Species"),
    plotOutput("plot")
  ),
  server = function(input, output, session) {
    output$plot <- renderPlot({
...
***This is a deviation from the original, simply bc I was not able to create the images.***
...
This requires getting your head around a key distinction that you haven't had to think about before: the different between a data-variable and an env-variable.
...
This is the last chapter in the "Shiny in Action" part of the book.
...
I had original considered a lot of this to be related to quosures, expressions, and the lovely `!!` bang bang. This chapter primarily took the variables and used basic techniques to incorporate the variables into the server side of the shiny app. These techniques may be useful in creating a shiny app, however since the output of shiny input is always a quoted string, you shouldn't have to worry about more complex NSE.
