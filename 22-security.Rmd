...
The following section lists six packages for configuration, authentication, and storage of login credentials. There are several options to choose from based on your need and design of the app.
...
3 - A simple and secure authentication mechanism for single ‘Shiny’ applications is **[{shinymanager}](https://datastorm-open.github.io/shinymanager/)**
...
4 - **[{shinyauthr}](https://paulc91.github.io/shinyauthr/)** This package is very useful for providing key function to use. 
...
These are the main functions to use which invisibly call JavaScript with `shinyjs::useShinyjs()` internally:
...
Also it provides a method for **cookie-based automatic login**, which means that the system can store the credentials for some time established by the creator of the app. 
...
5 - **[{sodium}](https://github.com/jeroen/sodium)** It is software library for encryption, decryption, signatures, password hashing and more.
...
In general what happens in an app when setting up a login credentials is described below.
...
Shiny inputs use **client-side validation** when accessing an API or database in R. It is often necessary to [provide credentials](https://www.r-bloggers.com/2019/03/how-to-avoid-publishing-credentials-in-your-code/) such as a login name and password. 


---------------------------------


In this example is shown for simplicity an application of the {shinyauthr}; the credentials are located inside the `server()`, they should be in a `config.yml` file and then retrieved for use.
...
Examples of Shiny inputs using **client-side validation** (input performed by JavaScript in the browser) where is possible for an attacker to send unexpected values:
...
What about errors, holes, dangerous function combinations, selective characters such as special characters that are not recognizable by the system language so that it can be filled with strange values?
...
2 - Other kind of possibilities for error chain generator are given by the **model formula**, the formula can executes an extra command `print()`
...
3 - Third warning comes from the {glue} package where the book in this case suggest to pay attention to: 

    glue::glue("{title}-{print('Hi'); number}")
    
and suggest to use `glue_safe()` instead which releases an error
...
```{r, out.width='25%', fig.align='center', fig.cap='https://xkcd.com/327/'}
...
all access to the apps is over SSL, and you can configure your app to require authentication prior to anyone accessing it, also every account has its own sub-directory structure.
...
Securing an **app** might require some extra work around all the information available as well as all the extra available packages and functions that can be used for authenticate and avoid wrongful combination of functions/commands.

This might be the case for apps containing sensitive information or for those one letting the users access to restricted data.
